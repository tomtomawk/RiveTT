...
11. Les murs, sols, plafonds, toits, garde-corps, escaliers et cartouches sont des types système : les énumérer avec `list_system_types` et les dupliquer avec `duplicate_system_type`.
...
- *Configurer pour Claude (config + skill ; import du ZIP dans Personnaliser > Skills)* ;
...
dépendrait de chaque implémentation ; celle-ci ne dépend que du classement `toolReadOnly`
...
| Écritures sans `dryRun` | **36** sur 139 — `execution.supportsDryRun` le dit par outil, et le serveur refuse `dryRun: true` sur les autres au lieu de les exécuter |
