...
public sealed class ShowStatusCommand : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        var dialog = new TaskDialog("MCPRVTT27")
        {
    ...
    private static void RevealInExplorer(string path)
    {
        ...
        {
            TaskDialog.Show("MCPRVTT27", $"Ouverture impossible : {exception.Message}");
        }
    }
}
...
internal static class RibbonWriteAccess
{
    ...
    {
        var policy = RevitCortexApp.Instance?.Session?.WriteAccess;
        if (policy == null)
        {
            TaskDialog.Show("MCPRVTT27",
        ...
        System.Diagnostics.Trace.WriteLine(
            $"[MCPRVTT27] Write access set to {writesAllowed} from the ribbon.");
    }
}
