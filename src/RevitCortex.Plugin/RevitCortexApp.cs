...
public sealed class RevitCortexApp : IExternalApplication
{
    ...
    public Result OnStartup(UIControlledApplication application)
    {
        ...
        try
        {
            ...
            {
                System.Diagnostics.Trace.WriteLine("[MCPRVTT27] Tools assembly could not be loaded.");
        ...
        {
            System.Diagnostics.Trace.WriteLine($"[MCPRVTT27] Startup failed: {ex}");
            return Result.Failed;
        }
    }
    ...
    public Result OnShutdown(UIControlledApplication application)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"[MCPRVTT27] Shutdown error: {ex.Message}");
        }
        return Result.Succeeded;
    }
    ...
    private static Assembly? LoadToolsAssembly()
    {
        try
        {
            ...
            return Assembly.LoadFrom(System.IO.Path.Combine(directory, "MCPRVTT27.Tools.dll"));
        }
        catch (Exception ex)
        {
            System.Diagnostics.Trace.WriteLine($"[MCPRVTT27] Could not load tools: {ex.Message}");
            return null;
        }
    }
}
