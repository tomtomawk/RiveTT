...
public sealed class RevitNamedPipeService : IDisposable
{
    ...
    {
        _router = router;
        PipeName = $"MCPRVTT27.Revit.{Process.GetCurrentProcess().Id}";
    }
    ...
    private async Task ListenAsync()
    {
        while (!_shutdown.IsCancellationRequested)
        {
            ...
            {
                Trace.WriteLine($"[MCPRVTT27] Named-pipe listener error: {ex.Message}");
            }
        }
    }
    ...
    private string ProcessRequest(string requestJson)
    {
        ...
        {
            Trace.WriteLine($"[MCPRVTT27] Pipe request failure: {ex}");
