...
public class RiveTTResultTests
{
    [Fact]
    public void Ok_SetsSuccessTrue_AndDataPopulated()
    {
        var result = RiveTTResult<string>.Ok("hello");
    ...
    {
        var result = RiveTTResult<string>.Fail(
        ...
        Assert.Equal(RiveTTErrorCode.ElementNotFound, result.Error!.Code);
    ...
    public void Fail_WithContext_SerializesCorrectly()
    {
        ...
        var result = RiveTTResult<object>.Fail(
            RiveTTErrorCode.ElementNotFound, "Not found", context: ctx);
    ...
    {
        var result = RiveTTResult<int>.Ok(42);
        var json = JsonConvert.SerializeObject(result);
