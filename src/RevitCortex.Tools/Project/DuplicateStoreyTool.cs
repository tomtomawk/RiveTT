...
public sealed class DuplicateStoreyTool : ICortexTool
{
    ...
    public CortexResult<object> Execute(JObject input, CortexSession session)
    {
        ...
        using var group = new TransactionGroup(doc, "MCPRVTT27: Duplicate Storey");
        group.Start();
        try
        {
            if (upperLevels.Count > 0)
            {
                using var moveTx = new Transaction(doc, "MCPRVTT27: Shift Upper Levels");
            ...
            using (var levelTx = new Transaction(doc, "MCPRVTT27: Create Target Level"))
            {
            ...
            using (var copyTx = new Transaction(doc, "MCPRVTT27: Copy Storey Elements"))
            {
